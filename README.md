...
| [`examples/custom-agents/`](examples/custom-agents/) | Inject custom AI agents into a running server via `ConfigUpdate` (GET-merge-PATCH), verify via `/agent`, then reference one by name in a session. | `go run examples/custom-agents/main.go` |
